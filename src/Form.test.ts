...
test('should generate form (set url, input and textarea)', () => {
  ...
  const expected = '<form method="post" action="/submit"><label for="name">Name</label><input name="name" type="text" value="Franky"><label for="job">Job</label><textarea name="job" rows="50" cols="50">Engineer</textarea></form>';
  expect(result).toBe(expected);
});
...
test('should generate form (set url, input, textarea and submit button with default text)', () => {
  ...
  const expected = '<form method="post" action="/submit"><label for="name">Name</label><input name="name" type="text" value="Franky"><label for="job">Job</label><textarea name="job" rows="50" cols="50">Engineer</textarea><input type="submit" value="Save"></form>';
  expect(result).toBe(expected);
});
