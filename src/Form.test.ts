...
test('should generate form (set url, an input)', () => {
  ...
  const expected =
      '<form method="post" action="/submit"><input name="name" value="Franky" type="text"></form>';
  expect(result).toBe(expected);
});
...
test('should generate form (set url, two inputs)', () => {
  ...
  const expected =
      '<form method="post" action="/submit"><input name="name" value="Franky" type="text"><input name="age" value="18" type="text"></form>';
  expect(result).toBe(expected);
});
...
test('should generate form (set url, an input and a textarea)', () => {
  ...
  const expected =
      '<form method="post" action="/submit"><input name="name" value="Franky" type="text"><textarea name="job" as="textarea" rows="50" cols="50">Engineer</textarea></form>';
  expect(result).toBe(expected);
});
